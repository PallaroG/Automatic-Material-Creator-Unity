...
- *Janela não aparece*: confira se o script está em `Assets/Editor/`.  
- *URP Lit não encontrado*: verifique o pacote URP no projeto; ou arraste um Template Material com shader URP.  
- *Normal não aplicado*: ative `Force normal map importer`, o script reimporta como NormalMap.  
- *Nada é criado*: confirme presença de UDIMs (ex.: `_1001` ou `.1001`) nos nomes.

## Como funciona (resumido)
1. Varre `Input Folder` por `Texture2D`.  
2. Ignora arquivos com `roughness`.  
3. Para cada arquivo detecta UDIM (`1001`, `1002`, ...).  
4. Agrupa por UDIM e associa mapas às propriedades do material (tentativa por várias propriedades comuns para compatibilidade URP/Standard).  
5. Cria `1001.mat`, `1002.mat`, ... com shader URP/Lit ou Template fornecido.

## Contribuição
Fork, faça melhorias (ex.: packing de canais, sufixos extras, naming collision handling) e envie pull request. 👍

## Licença
Recomendo MIT — sinta-se à vontade para usar/alterar e atribuir conforme desejar.
